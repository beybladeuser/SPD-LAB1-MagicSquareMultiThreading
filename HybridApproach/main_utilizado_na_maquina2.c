...
omp_lock_t* init_locks(int nThreads)
{

	omp_lock_t* lock = (omp_lock_t*)malloc((nThreads) * sizeof(omp_lock_t));
...
int main(int argc, char* argv[])
{
	...
	Matrix_file_read_data data = make_matrix_data("/home/dados-SPD/inputs/", argv[1], ".txt");
	...
	if (world_rank == 0)
	{
		...
		while (i < world_size - 1)
		{
			...
			//unsigned long long* send_buffer = (unsigned long long*)malloc(matrix_size * current_chunk_lines * sizeof(unsigned long long));
	...
	else
	{
		...
		int tID;
		omp_lock_t* wait_lock = init_locks(nThreads);
		unsigned long long line_sum = 0;
		unsigned long long line_sum_segment = 0;
		#pragma omp parallel shared(true_inicial_i, current_chunk_lines, diag_sums, column_sums, recv_buffer, matrix_size, line_sum, result, magic_const) private(tID, line_sum_segment)
		{
			tID = omp_get_thread_num();
			unsigned long long standard_cols_to_calc = matrix_size / nThreads;
			unsigned long long cols_to_calc = standard_cols_to_calc;
			if (tID == nThreads - 1)
			{
				cols_to_calc += matrix_size % nThreads;
			}
			
			for (unsigned long long i = true_inicial_i; i < true_inicial_i + current_chunk_lines && result != 2; i++)
			{
				line_sum_segment = 0;
				if (tID == 0)
				{
				...
				for (unsigned long long j = tID * standard_cols_to_calc; j < tID * standard_cols_to_calc + cols_to_calc; j++)
				{
					unsigned long long current_entry = recv_buffer[(i - true_inicial_i) * matrix_size + j];
					line_sum_segment += current_entry;
					column_sums[j] += current_entry;
				}
				
				#pragma omp critical (update_line_sum)
				{
					line_sum += line_sum_segment;
				}
				...
				if (tID == 0 )
				{
					result = line_sum != magic_const ? 2 : result;
					line_sum = 0;
					//printf("result=2 because line %llu value is %llu instead of %llu\n", i, line_sum, magic_const);
				}
		...
		if (world_rank != 1)
		{
			...
			if (result != 2)
			{
				#pragma omp parallel shared(matrix_size, column_sums, nThreads) private(tID)
				{
					tID = omp_get_thread_num();
					unsigned long long standard_cols_to_calc = matrix_size / nThreads;
					unsigned long long cols_to_calc = standard_cols_to_calc;
					...
					for (unsigned long long i = tID * standard_cols_to_calc; i < tID * standard_cols_to_calc + cols_to_calc; i++)
					{
		...
		else
		{
			if (result != 2)
			{
				...
				{
					result =  1;
					//printf("diag0 is %llu, diag1 is %llu instead of %llu\n", diag_sums[0], diag_sums[1], magic_const);
				}

				#pragma omp parallel shared(matrix_size, magic_const, column_sums, result, nThreads) private(tID)
				{
					tID = omp_get_thread_num();
					unsigned long long standard_cols_to_calc = matrix_size / nThreads;
					unsigned long long cols_to_calc = standard_cols_to_calc;
					...
					for (unsigned long long i = tID * standard_cols_to_calc; i < tID * standard_cols_to_calc + cols_to_calc; i++)
					{
						if (column_sums[i] != magic_const)
						{
							...
							{
								result = 2;
								//printf("result=2 becausecol %llu value is %llu instead of %llu\n", i, column_sums[i], magic_const);
							}
						}
